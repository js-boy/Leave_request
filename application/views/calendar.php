	...
	<div class="flex flex-col justify-center items-center">
		<div class="traveling-box w-full">
			<div class="flex justify-between items-center w-25">
				<h1 class="text-2xl font-bold flex-grow">Calendrier des congés</h1>
				<div class="flex items-center">
					<form action="<?php echo site_url('Calendar/date_input');?>" method="post" >
						<input type="date" class="border border-gray-200 p-2 rounded-lg mr-5" name="date_input">
						<button class="bg-[#a6c1ee] text-white px-5 py-2 rounded-full hover:bg-[#87acec]">Rechercher</button>
					</form>
				</div>
			</div>
		</div>
		<div class="traveling-box mb-48 w-full">
			<div class="flex justify-between items-center">
				<h1 class="text-2xl font-bold">Liste de conges</h1>
			</div>
			<div>
				<?php if (!empty($form_request)) : ?>
				<table class="table-auto divide-y divide-gray-200 w-full text-white">
					<thead class="bg-white">
						<tr>
							<th class="text-black">#</th>
							<th class="text-black">Nom</th>
							<th class="text-black">Date du debut</th>
							<th class="text-black">Date de fin</th>
						</tr>
					</thead>
					<tbody class="bg-black">
					<?php $form_request = $this->db->get('form_request')->result();
					foreach($form_request as $key => $request) :?>
						<tr>
							<td class="px-4 py-2 border"><?php echo $key + 1 ; ?></td>
							<td class="px-4 py-2 border"><?php echo $request->first_name.' '.$request->last_name; ?> </td>
							<td class="px-4 py-2 border"><?php echo $request->date_debut; ?></td>
							<th class="px-4 py-2 border"><?php echo $request->date_find; ?></th>
						</tr>
					<?php endforeach; ?>
					</tbody>
				</table>
